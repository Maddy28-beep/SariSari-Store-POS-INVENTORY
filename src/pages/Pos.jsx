  ...
  return (
    <Layout header={
      <div className="d-flex justify-content-between align-items-center">
        <h2 className="h4 mb-0">🛒 POS</h2>
        <span className="text-secondary">Cashier: {profile?.name}</span>
      </div>
    }>
            ...
            <div className="card-body">
              <div className="row g-2">
                ...
                <div className="col-md-5 position-relative" ref={searchBoxRef}>
                  ...
                  <input
                    type="text" className="form-control" placeholder="Search by name (for items without a barcode)"
          ...
          <div className="card">
            <div className="table-responsive">
              <table className="table table-hover mb-0 align-middle">
                ...
                <tbody>
                  {cart.length === 0 ? (
                    <tr><td colSpan={5} className="text-center text-secondary py-5">Scan or search a product to begin.</td></tr>
                  ) : cart.map((item, index) => (
                    <tr key={item.productId} className="pos-cart-row">
                      <td>{item.name}</td>
                      <td>
                        <div className="input-group input-group-sm">
                          <button className="btn btn-outline-secondary" onClick={() => updateQty(index, -1)}>-</button>
                          <input
          ...
          <div className="card">
            <div className="card-body">
              ...
              <div className="btn-group w-100 mb-3">
                {['cash', 'gcash', 'other'].map((pm) => (
                  <button
                    key={pm} type="button"
                    className={`btn ${paymentMethod === pm ? 'btn-primary' : 'btn-outline-primary'}`}
              ...
              <button className="btn btn-success btn-lg w-100" disabled={!canCheckout || submitting} onClick={handleCheckout}>
                {submitting ? 'Processing…' : 'COMPLETE SALE'}
              </button>
              {checkoutError && <div className="small text-danger mt-2">{checkoutError}</div>}
            </div>
          </div>
        </div>
      </div>
